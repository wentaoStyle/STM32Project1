...
void lcd_DrawPicture(u16 StartX,u16 StartY,u8 Dir,u8 *pic)
{
  ...
  //打点
  LCD_WR_REG(0x0202);				          	//写数据到显示区			 REG往哪写 Data写什么
			//202为GRAM显存  写存的地址 
...
void LCD_WR_CMD(unsigned int index,unsigned int val)
{								 //Bank1_LCD_C显存写命令的地址 0x60000000
	*(__IO uint16_t *) (Bank1_LCD_C)= index; //要写入的寄存器的信息	
...
void LCD_test(unsigned int temperature,unsigned int humidity,unsigned int people)
{
	...
	lcd_wr_zf(100,60,80,32,63489,1,zf4);  		//显示温度文字
	lcd_wr_zf(100,120,80,32,63489,1,zf5);  		//显示湿度文字

    lcd_wr_zf(100,0,280,32,color1,1,zf3);  		//显示班级姓名学号
	//往显存送280x32个bit位     	

	if(people == 1){
		lcd_wr_zf(100,180,80,32,63489,1,zf6);  		//显示是否有人
	...
	lcd_wr_zf(222,60,40,32,63489,1,zf7);  		//显示°C
	lcd_wr_zf(222,120,40,32,63489,1,zf8);  		//显示%RH
